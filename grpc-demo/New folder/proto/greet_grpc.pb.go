...
func (x *greetServiceSayHelloBidirectionalStreamingClient) Recv() (*MessagesList, error) {
	m := new(MessagesList)
	if err := x.ClientStream.RecvMsg(m); err != nil {
		return nil, err
	}
...
func (x *greetServiceSayHelloBidirectionalStreamingServer) Send(m *MessagesList) error {
	return x.ServerStream.SendMsg(m)
}
